function estimate_reflection_reconstruction_using_testing_data(training_testing_dir, conversion_matrix_dir, Pdegree)


filename=fullfile(training_testing_dir, 'rgb_image_test.mat');
var_name='rgb_img_te';
%------------------------------------------------------------------------
...
estimated_reflection_test = rgb_data * T;

mat_file=sprintf('estimated_reflection_test_deg=%d.mat',Pdegree);

filename=fullfile(training_testing_dir, mat_file);
save(filename,'estimated_reflection_test' );


msg=sprintf('Estimation is done successfully with degree of ploynomial = %d', Pdegree);
disp(msg);
